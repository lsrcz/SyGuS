...
def addNewValue(var, Set, VarTable, samples, hashTable):
    resultVar = VarTable["__result"]
    ...
    for sample in samples:
        ...
        for arg in VarTable:
            if arg in sample:
                solver.add(VarTable[arg] == sample[arg])
        solver.check()
        model = solver.model()
    ...
    if hashIndex not in hashTable:
        ...
        for otherVar in hashTable[hashIndex]:
            solver.push()
    ...
    print(var)
    hashTable[hashIndex].append(var)
    Set.append(var)
    return True
...
def getPossibleValue(Operators, Expr, Terminals):
    ...
    if returnType == 'Bool':
        ...
        for terminal in Terminals['Int']:
            addNewValue(terminal, Value[0], argVarTable, Samples, hashTable)
        print(Value)
        while True:
            resultSet += Value[depth]
            print(resultSet)
            ...
            for operatorType in Operators:
                ...
                for lsize in range(depth):
                    for rsize in range(depth - lsize):
                        for operator in operatorType[0]:
                            for lTerm in Value[lsize]:
                                for rTerm in Value[rsize]:
                                    addNewValue([operator, lTerm, rTerm], Value[depth], argVarTable, Samples, hashTable)
...
if __name__ == '__main__':
    ...
    times = 1
    PossibleTerm = getPossibleValue(Operators, bmExpr, Terminals)