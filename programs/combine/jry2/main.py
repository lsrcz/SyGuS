import sys
import jry2.sexp as sexp
import pprint
import random
import jry2.translator as translator
from jry2.possibleterm import *
from jry2.task import SynthTask
from z3 import *
...
def getTermCondition(Expr, TermInfo, currentTerm, RemainTerms, ConsTable, VarTable):
    ...
    while True:
        ...
        reducedCondition = reduceCons(checkSolver, BestCons, [], VarTable, SynFunExpr[2], functionArgs)
...
def main(return_dict):
    ...
    if len(PossibleTerm) == 1:
        return_dict["jry2"] = getCode([[[], PossibleTerm[0]]], SynFunExpr)
        return
    ...
    return_dict["jry2"] = resultCode
    # assert(checker.check(resultCode) is None)

if __name__ == '__main__':
    return_dict = dict()
    main(return_dict)
    print(return_dict["jry2"])
